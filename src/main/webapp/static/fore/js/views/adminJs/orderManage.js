...
function initBind($http, $scope, $timeout) {
    ...
    $('#table_id_example').on('click', '.update', function () {
        ...
        $.ajax({
            ...
            success: function (result) {
               // result = $.parseJSON(result);
                $("#rIdT").val(result.extend.rId);
                $("#oNameT").val(result.extend.oName);
                $("#oPhoneT").val(result.extend.oPhone);
                $("#oAddressT").val(result.extend.oAddress);
    ...
    $('#table_snack_example').on('click', '.deleteOrderDetial', function () {
        ...
        swal({
            ...
            $.ajax({
                url: '/snack' + '/admin/order/deleteOrderdetail',
                data: {
                    "dId": params,
                    "oId": oId
                },
